...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_place is not hive:
            if len(current_place.bees) > 0 and count >= self.min_range and count <= self.max_range:
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        bee = list(self.place.bees)
        for _ in bee:
            Insect.reduce_armor(_, self.damage)
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, colony):
        ...
        if self.impostor: 
            ...
            location = self.place
            while type(location.ant) != QueenAnt:
                double_damage +=location.ant
                if type(location.ant) == BodyguardAnt:
                    double_damage += location.ant.damage
