...
def apply_primitive(procedure, args, env):
    ...
    4
    """
    python_list_of_arguments = list(args)
    try: 
        if procedure.use_env: 
            python_list_of_arguments.append(env)
        return procedure.fn(*python_list_of_arguments)
    except TypeError: 
        raise SchemeError()
...
def do_let_form(vals, env):
    ...
    for elem in range(len(bindings)):
        names = Pair(bindings[elem].first, names)
        values = Pair(scheme_eval(bindings[elem].second.first, env), values)
...
def do_if_form(vals, env):
    ...
    try:
        if scheme_true(predicate):
            return vals[1]
        elif not scheme_true(predicate):
            return vals[2]
    except IndexError:
        return okay
...
def check_formals(formals):
    ...
    if not scheme_listp(formals):
        raise SchemeError(str(formals) + ' not a well-formed list of symbol')

    invalid_list = []
    for i in formals: 
        if i in invalid_list: 
            raise SchemeError(str(formals) + ': repeated symbols')
        elif not scheme_symbolp(i):
            raise SchemeError(str(i) + ': parameters is invalid')
