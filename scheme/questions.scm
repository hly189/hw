...
(define (merge comp list1 list2)
  (if (eq? list1 nil) list2
    (if (eq? list2 nil) list1
...
(define (list-partitions total max-pieces max-value)
  (define empty '())
  (define (helper total max-pieces max-value couting)
    (cond
      ((eq? total 0) (list couting))
      ((or (< total 0) (eq? max-pieces 0) (< max-value 1)) empty) 
      (else (append
              (helper (- total max-value) (- max-pieces 1) max-value (append couting (list max-value)))
              (helper total max-pieces (- max-value 1) couting)))))
  (helper total max-pieces max-value empty)
  )
...
(let 
  ((x (flatten (map (children tree) tree-sums))))
  (map x (lambda (x) (+ x (entry tree)))))))
