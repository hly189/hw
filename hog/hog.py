...
def roll_dice(num_rolls, dice=six_sided):
    ...
    total = 0 # total scores 
    rule = False # Pig out rule 
    k = 0 # use to keep track of num_rolls 
    
    while k<num_rolls:
        k=k+1
        outcome = dice()
        if outcome == 1: # if outcome equal 1,the Pig out rule is true
            rule = True
        else:
            total = total + outcome
    if rule:5
...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while (score<goal and opponent_score<goal):
        ...
        if score == 2*(opponent_score) or score == 0.5*(opponent_score):
            score, opponent_score = opponent_score, score
        else: 
            score, opponent_score = score, opponent_score
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    maxval = 0
    while n <11:
        """ I did assign make_averaged to vairable outside the loop, but
        it failed the doctest"""
        averaged = make_averaged(roll_dice, num_samples=1000)(n, dice)
        if averaged > maxval:
            maxval = averaged
            idx  = n
        print(str(n), 'dice scores', str(averaged), 'on average')
        n = n+1
    return idx
...
def run_experiments():
    ...
    if False: # Change to False to test always_roll(8)
...
def bacon_strategy(score, opponent_score):
    ...
    """
    dice = select_dice(score, opponent_score)
    taketurn = take_turn(0, opponent_score, dice)
    if taketurn >= BACON_MARGIN:
        return 0
...
def swap_strategy(score, opponent_score):
    ...
    """
    dice = select_dice(score, opponent_score)
    max_score = score + max((opponent_score //10), (opponent_score%10)) +1
    if max_score == 0.5*opponent_score:
        return 0
    elif max_score == 2*opponent_score:
        return BASELINE_NUM_ROLLS
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if (opponent_score>90 and score<65): #if we are at disadvantage, roll more dice
        return 10

    elif score>94 and opponent_score>40: #if we are abover certain scores, we only need to use the free bacon rule to gareentee our win
        return 0
    elif score>96 and opponent_score>30:
        return 0
    elif score>97 and opponent_score>20:
        return 0
    elif score>98 and opponent_score>2:
        return 0

    elif (score + max(opponent_score//10, opponent_score%10)+1) == 0.5*opponent_score: #use the swap rule
        return 0
    elif (score + max(opponent_score//10, opponent_score%10)+1) == 2*opponent_score:
        return BASELINE_NUM_ROLLS

    elif 2*score+1 == opponent_score: #try to get 1 if we only need one to swap the score
        return 10
    elif 2*opponent_score+1 == score: #try to avoid 1 if the opponent only need one to swap
        return 0
    else:
        return bacon_strategy(score,opponent_score)
